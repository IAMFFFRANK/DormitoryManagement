...
<div class="panel admin-panel margin-top">
  ...
  <div class="body-content">
    <form method="post" class="form-x" action="/ssm_dorm_manage/student/exUpdate.do" enctype="multipart/form-data"> 
    <input type="hidden" name="id" value="${obj.id}">  
       ...
       <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input w50" name="nickame" value="${obj.nickName }" data-validate="required:请输入真实名" />         
          <div class="tips"></div>
        </div>
      </div> 
       <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input w50" name="studentId" value="${obj.studentId }" data-validate="required:请输入学号" />         
          <div class="tips"></div>
        </div>
      </div> 
       <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input w50" name="jiguan" value="${obj.jiguan }" data-validate="required:请输入籍贯" />         
          <div class="tips"></div>
        </div>
      </div>
      <div class="form-group">
          ...
          <div class="field">
            <select name="gradeId" class="input w50">
              <option value="">请选择年级</option>
               <c:forEach items="${gradeList}" var="data" varStatus="l">
               		<c:if test="${data.id == obj.gradeId}">
       ...
       <div class="form-group">
          ...
          <div class="field">
            <select name="teamId" class="input w50">
              <option value="">请选择班级</option>
               <c:forEach items="${teamList}" var="data" varStatus="l">
               	   <c:if test="${data.id == obj.teamId}">
	                	<option value="${data.id}" selected="selected">${data.name }</option>
	               </c:if>
	                <c:if test="${data.id != obj.teamId}">
	                	<option value="${data.id}">${data.name }</option>

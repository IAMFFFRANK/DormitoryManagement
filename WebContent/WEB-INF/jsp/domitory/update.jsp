...
<div class="panel admin-panel margin-top">
  ...
  <div class="body-content">
    <form method="post" class="form-x" action="/ssm_dorm_manage/domitory/exUpdate.do" enctype="multipart/form-data"> 
    <input type="hidden" name="id" value="${obj.id}">  
        ...
        <div class="field">
          <input type="text" class="input w50" name="domitoryNo"  data-validate="required:请输入宿舍号" value="${obj.domitoryNo }"/>         
          <div class="tips"></div>
        </div>
      </div> 
	 ...
	 <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input w50" name="tel"  data-validate="required:请输入宿舍电话" value="${obj.tel }"/>         
          <div class="tips"></div>
        </div>
      </div> 
          ...
          <div class="field">
            <select name="type" class="input w50">
              <option value="">请选择宿舍类型</option>
					<option value="1" <c:if test="${obj.type == 1 }"> selected="selected"</c:if>>单人间</option>
					<option value="2" <c:if test="${obj.type == 2 }"> selected="selected"</c:if>>双人间</option>
					<option value="4" <c:if test="${obj.type == 4 }"> selected="selected"</c:if>>四人间</option>
					<option value="6" <c:if test="${obj.type == 6 }"> selected="selected"</c:if>>六人间</option>
            </select>
            <div class="tips"></div>
          </div>
        </div>
